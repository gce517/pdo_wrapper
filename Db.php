...
class Db extends PDO {
    ...
    public function insert($table, $data) {
        $q = $this->buildQuery(1, $table, $data);
    ...
    public function update($table, $data, $where_and, $where_or) {
        $q = $this->buildQuery(2, $table, $data, $where_and, $where_or);
     ...
     * @param int $action (required) insert 1 or update 2
     * @param string $table (required) the name of the table
     * @param array $data (required) array of columns requested
     * @param array $where_and (optional) column = value as a string (['{field} {operand}' => ':parameter']). required when using $where_or
     * @param array $where_or (optional) column = value as a string
     * @return array of sql and arguments
     */
    private function buildQuery($action, $table, $data, $where_and = null, $where_or = null) {
        $fields = [];
        $args   = [];
        $sql    = ($action == 1 ? 'INSERT INTO' : 'UPDATE');
        $where  = '';
        ...
        if ($where_and != null) :
            ...
            foreach ($where_and as $key => $value) :
                ...
                $key     = $split[0];
                $operand = $split[1];
                $where   .= ($counter == 0 ? 'WHERE' : 'AND') . ' `' . $key . '` ' . $operand .' :' . $key . '_a' . PHP_EOL;
                $args[':' . $key . '_a'] = $value;
                $counter++;
            endforeach;
            if ($where_or != null) :
